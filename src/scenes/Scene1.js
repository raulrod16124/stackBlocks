...
class Scene1 extends Scene {
	...
	events(){
		...
		Observer.on(EVENTS.STACK, (new_box) => {
			...
			// Remove the principal block
			this.boxes_group.remove(this.getLastBox());

			// Space tu cut the block
			const actual_base_cut = new SliceBox(new_box);
		...
		Observer.on(EVENTS.GAME_OVER, () => {
			if(!this.game_over){
				this.stack_points = 0;
				const tween_game_over = new TWEEN.Tween(this.getLastBox().position)
					.to({
	...
	resetGroup(){
		this.boxes_group.children.map((box, index) => {
